...
import { API_HOST, RELIGION_STATUS } from "../../../../config/apiConfig";
...
const ReligionMaster = () => {
  ...
  const fetchReligionData = async (flag = 0) => {
    try {
      setLoading(true);
      const response = await getRequest(`${RELIGION_STATUS}/all/${flag}`);
  
      if (response && response.response) {
        const transformedData = response.response.map(religion => ({
          id: religion.id,
          code: religion.code,
          name: religion.name,
        ...
        setReligionData(transformedData);
  ...
  const filteredReligionData = religionData.filter(religion =>
    religion.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
    religion.code.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filteredReligionData.slice(indexOfFirstItem, indexOfLastItem);
    ...
    try {
      ...
      if (editingReligion) {
        ...
        const isDuplicate = religionData.some(
          (religion) =>
            religion.id !== editingReligion.id && // Exclude the current item
            (religion.code === formData.code ||
            religion.name === formData.name)
        );
    
        if (isDuplicate) {
          showPopup("Religion already exists!", "error");
          setLoading(false);
          return;
        }
        
        const response = await putRequest(`${RELIGION_STATUS}/update/${editingReligion.id}`, {
          code: formData.code,
          name: formData.name
        });
        ...
        if (response && response.response) {
          setReligionData(religionData.map(religion =>
            religion.id === editingReligion.id ? response.response : religion
          ));
        ...
        const response = await postRequest(`${RELIGION_STATUS}/create`, {
          code: formData.code,
          name: formData.name
        });
        
        if (response && response.response) {
          setReligionData([...religionData, response.response]);
      ...
      console.error("Error saving religion data:", err);
    ...
    if (confirmed && confirmDialog.religionId !== null) {
      try {
        ...
        const response = await putRequest(
          `${RELIGION_STATUS}/change/${confirmDialog.religionId}?status=${confirmDialog.newStatus}`
        );
        ...
        if (response && response.response) {
          ...
          showPopup(`Religion ${confirmDialog.newStatus === "y" ? "activated" : "deactivated"} successfully!`, "success");
        }
      } catch (err) {
            ...
            {loading ? (
                ...
                <div className="table-responsive packagelist">
                  <table className="table table-bordered table-hover align-middle">
                    ...
                    <tbody>
                      {currentItems.length > 0 ? (
                        ...
                        <tr>
                          <td colSpan="4" className="text-center">No religion data found</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
                  {filteredReligionData.length > 0 && (
                     ...
                     <div>
                       <span>
                         Page {currentPage} of {filteredTotalPages} | Total Records: {filteredReligionData.length}
                       </span>
                     </div>
                ...
                <form className="forms row" onSubmit={handleSave}>
                  <div className="form-group col-md-4">
                    <label>Religion Code <span className="text-danger">*</span></label>
                    <input
                      type="text"
                  ...
                  <div className="modal-dialog">
                    <div className="modal-content">
                      <div className="modal-header">
                        <h1 className="modal-title fs-5" id="staticBackdropLabel">Religion Reports</h1>
                        <button type="button" className="btn-close" onClick={() => setShowModal(false)} aria-label="Close"></button>
                      </div>
                      <div className="modal-body">
                        <p>Generate reports for religion data:</p>
                        <div className="list-group">
                          <button type="button" className="list-group-item list-group-item-action">Religion Distribution Report</button>
                          <button type="button" className="list-group-item list-group-item-action">Active/Inactive Religion Report</button>
                        </div>
                      </div>
                ...
                <div className="modal d-block" tabIndex="-1" role="dialog">
                  <div className="modal-dialog" role="document">
                    <div className="modal-content">
                      ...
                      <div className="modal-body">
                        <p>
                          Are you sure you want to {confirmDialog.newStatus === "y" ? 'activate' : 'deactivate'} <strong>{religionData.find(religion => religion.id === confirmDialog.religionId)?.name}</strong>?
                        </p>
                      </div>
                      <div className="modal-footer">
